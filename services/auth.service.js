...
export class AuthService {
    ...
    async login(req, res) {
        ...
        try {
            ...
            const user = await this.studentModel.findOne({ username }).session(session);
            if (!user) {
            return { status: 401, message: "Invalid username or password" };
            }
            ...
            : (user.password
                ? { username, password: decrypt(user.password.split(":")[1], user.password.split(":")[0]) }
                : { username, password });
        
            const isMatch = await this.comparePassword(credentials.username, credentials.password, res);
            ...
            return { status: 200, message: "Login successful", data: { token: token.token, user: hidePassword(user.toObject()) } };
        } catch (error) {
